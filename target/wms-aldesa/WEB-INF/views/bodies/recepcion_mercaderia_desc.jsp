...
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core"%>
	...
	<div class="col-lg-6 col col-lg-offset-3" >
		...
		<div class="row">
			<div class="col col-lg-4 col-lg-offset-4">
				<label for="frecepcion">Forma de Recepcion de Mercaderia</label>
		...
		<div class="row">
            ...
            <table class="table table-tripped" disabled>
                ...
                <c:forEach var="m" items="${observ}" varStatus="loop">
                    <tr>
                        <td>${ m.getCliente()}
		                <input type="hidden" name="cliente" value="${m.getCliente()}"/>
		                </td>
                        <td>${m.getNumeroInforme() }     </td>
						<td>${m.getNombre() }     </td>
						<td>
							<textarea name="observacion" class="form-control obs" style="width: 100%;" 
								placeholder="Observaciones...">${m.getObservaciones()}</textarea>
						</td>
                    </tr>
                </c:forEach>
			...
			<div class="col col-lg-3 col-lg-offset-1">
				...
				<script>
					var preview = function(){
						...
						var observaciones = '-'; //$("#observacion").val().trim();
						...
						*/
						var obser = document.getElementsByClassName("obs");
						var obs = "";
						for (var i=0; i < obser.length; i++) {
							obs = obs + "obs=" + obser[i].value;
							if(i<obser.length-1)
								obs=obs+"&";
						}
						url2 = "/wms-aldesa/web/reporte/recibida_simple/${deposito}/1/" + forma + "/" + recibido + "/" + entregado + "/" + observaciones + "/${cliente}/?"+obs;
						window.open(url2, '_');
