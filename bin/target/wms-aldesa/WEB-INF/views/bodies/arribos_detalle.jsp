		...
		<div class="container">
			<form method="post" id="guardar-detalles">
				...
				<script type="text/javascript">
					...
					var validarLote = function(valor){
						var v = valor;
						if(v.length==0)
							alert('El valor de lote esta vacío');
					};
					var validarFechaVto = function(valor){
						var v = valor;
						if(v.length==0)
							alert('El valor de fecha de vencimiento esta vacío');
					};
					...
					<c:forEach var="m" items="${pendientes}" varStatus="loop">
						<tr>
							...
							<td>
								...
								<c:if test="${ finalizado }">disabled</c:if>
								<c:if test="${ bulk==null }">disabled</c:if>
								onblur="validarLote(this.value)"
								/>
							</td>
							<td>
							
								<input type="date" name="fechavto" id="fechavto" class="form-control" 
								<c:if test="${ finalizado }">disabled</c:if>
								<c:if test="${ bulk==null }">disabled</c:if>
								onblur="validarFechaVto(this.value)"
